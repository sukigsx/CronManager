...
nombre_carpeta_repositorio="CronManager" #poner el nombre de la carpeta cuando se clona el repo para poder eliminarla
...
borrar_tarea() {
    ...
    CRON_CONTENT=$(crontab -l 2>/dev/null | grep -v '^\s*$' | grep -v '^#' | sed 's/^/   /')
    if [[ -z "$CRON_CONTENT" ]]; then
        echo -e "${rojo} No hay tareas programadas en el crontab de${borra_colores} $(whoami)"; sleep 3
    else
